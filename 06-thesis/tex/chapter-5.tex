...
\section{\label{sec:SSH} Su-Schrieffer-Heeger model with spin}
...
We investigate the phases of the model in Figure \ref{fig:normal-SSH-phase-diagram}. Importantly, we may have naivly expected the addition of a spin quantum number to the SSH to be trivial i.e. simply doubling the degrees of freedom of the original SSH model. Performing the calculation, however, reveals nontrivial magnetic effects at $\mu=0$.

\createfigure{SSH-unit-cell}{SSH unit cell}

\createfigure{normal-SSH-phase-diagram}{Normal SSH phase diagram with magnetism, staggered density and IPR.}

\createfigure{normal-SSH-LDOS-zero}{Normal SSH LDOS}

\createfigure{normal-SSH-LDOS-nonzero}{Normal SSH LDOS}

\createfigure{normal-SSH-LDOS-two-impurities}{Normal SSH LDOS two impurities}
...
\begin{eqnarray}
\phi_{x,\sigma, m}&=& (U_w-U_v)\hat c_{x, \sigma, m}^\dagger \hat c_{x, \sigma, m}\\
...
\createfigure{06-01-phase-diagram-perculation}{Building the spin-tripelt SSH phase diagram}
\createfigure{06-01-phase-diagram-no-impurity}{Spin-triplet SSH phase diagram without impurity}

\createfigure{Spin-triplet-SSH-LDOS}{Spin-triplet SSH LDOS}
\createfigure{Spin-triplet-SSH-magnetism}{Spin-triplet SSH magnetism}
\createfigure{Spin-triplet-SSH-staggered}{Spin-triplet SSH staggered density}
\createfigure{Spin-triplet-SSH-IPR}{Spin-triplet SSH IPR}
