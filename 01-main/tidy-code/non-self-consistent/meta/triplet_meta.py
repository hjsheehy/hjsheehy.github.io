...
i=0
def create_name(n_x,n_y,mu,s,delta,t,Delta,eta,V):
        return f'Non_unitary_triplet_{n_x:d}_{n_y:d}_{mu:0.2f}_{s:0.2e}_{delta:0.2e}_{t:0.2f}_{Delta:0.2e}_{eta[0]}_{eta[1]}_{eta[2]}_{V:0.2e}'.replace('0.00e+00','0')
...
# for Delta in np.linspace(Delta_bits[0],Delta_bits[1],Delta_bits[2],endpoint=True):
    ...
    # i+=1
n_y=n_x
confname = os.path.join(fileDir, 'output/'+create_name(n_x,n_y,mu,s,delta,t,Delta,eta,V))
