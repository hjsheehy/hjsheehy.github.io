...
def create_name(n,mu,s,delta,t,Delta,V,U,T):
        return f'Self-consistent_singlet_varying_mu_{n:d}_{mu:0.2f}_{s:0.2e}_{delta:0.2e}_{t:0.2f}_{V:0.2e}_{U:0.2e}_{T:0.2e}'.replace('0.00e+00','0')

for U in np.linspace(U_bits[0],U_bits[1],U_bits[2],endpoint=True):
    for mu in np.linspace(mu_bits[0],mu_bits[1],mu_bits[2],endpoint=True):
