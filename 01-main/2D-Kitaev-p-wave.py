...
for directory in [DATA_Delta,FIG]:
    if not os.path.exists(directory):
        os.makedirs(directory)

def model(mu,Uv,rho_shift):
...
def dependent_variables(bdg):
    ...
    fock=bdg.fock(atom_i='A', atom_f='B', hop_vector=[0,0])[0,0]
    gorkov=bdg.gorkov(atom_i='A', atom_f='B', hop_vector=[0,0])[0,0]

    return [hartree_A, hartree_B, fock, gorkov]
...
_print=False
Uvs=[-5.6,-4.2,-2.8,-1.4,0,1.4,2.8,4.2,5.6]
