...
data = np.load(glob.glob(os.path.join(data_location,confname)+'_DOS*')[0], allow_pickle=True)
...
def Main():
    ...
    n_x,n_y=np.shape(dos_map)
    
    text_DOS = (f'$\mathbf{{\Delta}}(\mathbf{{r}},\omega)$ map'
    '\n'
    f'$\omega=${eV:.2f}'
    '\n'
    f'$\epsilon={epsilon}$'
    ...
    newcmp = ListedColormap(cm.afmhot(np.linspace(0, 0.75, 256)))
    im = axs.imshow(
