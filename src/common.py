...
def create_seed(seed_index, seed_coord, INITIAL_FIELD, xlabel, ylabel, output_folder, V):

    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    temp_conf = f'''INITIAL_FIELD='{INITIAL_FIELD}'
...
def create_seeds(seed_coords, INITIAL_FIELDS, xlabel, ylabel, output_folder, V):
    ...
    for INITIAL_FIELD in INITIAL_FIELDS:
        for seed_index,seed_coord in enumerate(seed_coords):
            ...
            create_seed(seed_index=seed_index, seed_coord=seed_coord, INITIAL_FIELD=INITIAL_FIELD, xlabel=xlabel, ylabel=ylabel, output_folder=output_folder, V=V)

def create_germ(layer_coord, dx, dy, seed_index, seed_coord, layer_index, xlabel, ylabel, output_folder, V):
...
def create_germs(germ_coords, dx, dy, seed_index, seed_coord, layer_index, xlabel, ylabel, output_folder, V):

    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    for layer_coord in germ_coords:
        create_germ(layer_coord=layer_coord, dx=dx, dy=dy, seed_index=seed_index, seed_coord=seed_coord, layer_index=layer_index, xlabel=xlabel, ylabel=ylabel, output_folder=output_folder, V=V)
