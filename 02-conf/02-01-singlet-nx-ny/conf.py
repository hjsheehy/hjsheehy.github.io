...
########################## Configuration ############################
####################################################################
def CreateName(n_x,n_y,mu,t,V):
        return f'Singlet_{n_x:02d}_{n_y:02d}_{mu:0.2f}_{t:0.2f}_{V:0.2e}'.replace('0.00e+00','0')

def Main():
    ClearOutFolder(out_folder, silent)
    for n in n_list:
        n_x=n_y=n    
        confname = os.path.join(out_folder,CreateName(n_x,n_y,mu,t,V))
...
initial_hartree = np.kron(hartree_SO, np.eye(n_x*n_y))
initial_hartree = np.diag(initial_hartree)

initial_fock = np.diag(np.zeros([n_x*n_y*n_spins*n_orbs]))

gorkov_SO = np.kron(np.array([[0,+Delta],[-Delta,0]]), np.eye(n_orbs))
initial_gorkov = np.kron(gorkov_SO, np.eye(n_x*n_y))
